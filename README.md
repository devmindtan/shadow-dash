...
- **Training Room** ("Phòng Luyện Tập" button on the menu): a practice mode where you can't permanently die (a fatal hit heals back to full instead of ending the run) and every effect upgrade has its own +/- level dial (0-5), so you can jump straight to testing e.g. "Dư Chấn Lv.5" without grinding wave-clears for it. "+ Quái" / "+ Power-up" buttons spawn one on demand instead of waiting on the normal random timers.
...
`main.ts` had grown to ~700 lines mixing every concern, so it's split into plain modules (functions, not classes — no State-Pattern/Manager-class ceremony, this is a single-scene arcade game, not a multi-scene app):
- `src/input.ts` — keyboard movement state, `getMoveDirection()`.
- `src/orbs.ts` — the `Orb` type, spawn/split, and the per-frame movement-behavior branch (`drift`/`wobble`/`track`/`charge`/`split`).
- `src/powerups.ts` — the `PowerUp` type, spawn, idle pulse animation.
- `src/upgrades.ts` — `RunStats`, the 6 effect level-curve arrays, `applyUpgrade`/`getEffectLevel`/`setEffectLevel`/`pickRandomUpgrades`.
- `src/ui.ts` — every DOM reference, `worldToScreen`/`clampToBounds` (the viewport), HUD/Library/character-select/training-panel rendering, and the CSS-based visual effects (dash ghosts, hit-bursts).
- `src/main.ts` — the orchestrator: game-state flags, `tryDash`/`takeDamage` (these touch multiple systems at once, so they stay at the top level), `startGame`/`endGame`, and the render loop wiring everything together.

Shared runtime state (`playerPos`, `selectedCharacter`, `runStats`, `hp`, the `orbs`/`powerUps` arrays) is owned by `main.ts` and passed into module functions as explicit parameters — no hidden cross-module mutable globals, no dependency-injection ceremony.
...
- The player, shield ring, dash-trail ghosts, and hit-bursts are DOM elements, not Babylon meshes — deliberately, so nothing can look "3D" or tumble. `worldToScreen()` in `ui.ts` is the single conversion point between game-world coordinates and their on-screen `translate` position; `playerPos` (owned by `main.ts`) stays the source of truth for collision math against the Babylon-space orbs/power-ups.
...
- Effect-upgrade level curves are explicit arrays (`CHAIN_REFUND`, `MAGNET_RADIUS`, `MAGNET_PULL`, `SHOCKBURST_MULT`, `RETALIATE_MULT`, `AURA_MULT` in `src/upgrades.ts`), indexed `[level - 1]`, capped at `MAX_EFFECT_LEVEL` (5) — not a formula. `entities.ts`'s `levelDetails` text for each upgrade is written to match these arrays exactly; if you retune a value in `upgrades.ts`, update the matching line in `entities.ts` too (and vice versa), since nothing enforces that link automatically. `pickRandomUpgrades()` filters an upgrade out of the random pool once `acquiredUpgrades` reaches its `maxLevel`.
- `ORB_TYPES`'s `"track"` behavior is fundamentally different from the other three: it recomputes direction toward the player's *live* position every frame and integrates position incrementally, instead of the parametric `spawnX + dir*traveled` closed form the others use (see `updateOrbMovement()` in `orbs.ts`). Any future "reactive" orb mechanic (that needs to respond to live game state, not just elapsed time) should follow the same incremental-update branch, not the parametric one.
- The Library ("Thư Viện") screen is generated straight from `ORB_TYPES` and `UPGRADES` (`libraryCard()`/`initLibrary()` in `ui.ts`) — it can't drift out of sync with the data. Adding a name/description to a new orb or upgrade entry is enough; no separate Library content to maintain. It's tabbed (`.library-tab` + `.library-grid`), not a fixed two-column list, specifically so it keeps scaling as more entries get added.
...
- **Fixed bug**: Dash Nối Tiếp (`dashChainLevel`) only refunded cooldown when `killedCount > 0`, but Phantom's Phase dash effect never kills anything (it's pure evasion) — so the upgrade was silently 100% non-functional on Phantom unless also stacked with Dư Chấn. Fixed by counting a "near miss" (an orb within 2x the normal hit radius during the dash) as equivalent to a kill in the phase branch of `tryDash()`. Any *new* effect upgrade that triggers "on dash-kill" needs the same check: does it have a real trigger path for every `DashEffect`, including `"phase"`? Verified live in the Training Room (Phantom, Dash Nối Tiếp Lv.5, near-miss dash → cooldown refunded).
- Training Room's "can't die" is implemented as late as possible — in `takeDamage()`'s final `if (hp <= 0)` check, auto-healing to full instead of calling `endGame()` — specifically so it happens *after* the normal damage/Trả Đòn/Hồi Sinh code already ran. This means every on-hit effect still fires for real in the Training Room, not just the Dash-triggered ones.
