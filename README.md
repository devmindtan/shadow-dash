...
- **Library** screen ("Thư Viện" button on the menu), styled like an open album/handbook: a narrow "cover" panel (title + subtitle) beside a "page" with chapter tabs (Quái Vật / Nâng Cấp) and a numbered card grid — built directly from `ORB_TYPES`/`UPGRADES` so it can never drift out of sync with the actual game data, and scales cleanly as more entries get added (grid + tabs, not a fixed two-column list)
